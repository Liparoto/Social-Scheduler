...
- [ ] **Publish-in-flight race** (follow-up under the merge section): the worker reads
      `status='scheduled'`, loads state into memory, and only later writes `'publishing'`, so a
      guard that checks for `publishing` can let a delete through mid-send. Blocking unmerge on
      any `scheduled` send sidesteps this — but the real fix is worker-side conditional claiming,
      `UPDATE publications SET status='publishing' WHERE id=? AND status='scheduled'`, aborting
      if it updates 0 rows. That same claim also closes the two-daemon double-publish hole.
