...
-- a hue means a chosen colour reuses that exact derivation, so the contrast relationship
-- between the text, background and dot stays consistent for any hue picked — an
-- illegible combination is not possible. (This does not make the pill dark-mode aware —
-- channelColor()'s background is a fixed light tint regardless of theme.)
--
